...
def fused_leaky_relu(input, bias: Optional[nn.Parameter]=None, negative_slope: float = 0.2, scale: float = 2 ** 0.5):
    if bias is not None:
        rest_dim = [1, bias.shape[0]] + [1] * (input.ndim - bias.ndim - 1)
        return (
            F.leaky_relu(
                input + bias.reshape(rest_dim), negative_slope=negative_slope
            )
            * scale
        )
...
class ModulatedConv2d(nn.Module):
    ...
    def forward(self, input, style):
        ...
        if not self.fused:
            ...
            dcoefs = torch.tensor([0])
            if self.demodulate:
                w = weight.unsqueeze(0) * style.reshape(batch, 1, in_channel, 1, 1)
            ...
            if self.demodulate:
                out = out * dcoefs.reshape(batch, -1, 1, 1)
...
class Generator(nn.Module):
    ...
    def style_to_image(self, w: torch.Tensor):
        latent = w.unsqueeze(1).repeat(1, self.n_latent, 1)
...
def load_generator():
    ...
    checkpoint = "generator_weights.pt"
    if not os.path.exists(checkpoint):
        import gdown

        print("Pobieranie modelu...")
        url = "https://drive.google.com/file/d/1v1DvwPzEAZR5JUNX03HVvaASPu4HraDQ/view?usp=drive_link"
        gdown.download(url, checkpoint, quiet=True, fuzzy=True)

    generator = Generator(size, latent, n_mlp, channel_multiplier)
